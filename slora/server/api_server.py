...
async def generate(request: Request) -> Response:
    ...
    sys_len = request_dict.pop("sys_len")
    app = request_dict.pop("app")
    input99app = request_dict.pop("input99app")
    sys99app = request_dict.pop("sys99app")
    output99app = request_dict.pop("output99app")
    priorityfactor = request_dict.pop("priorityfactor")
    llmcalls = request_dict.pop("llmcalls")
    app_limit = request_dict.pop("app_limit")
    llmcalls_made = request_dict.pop("llmcalls_made")
    interaction_id = request_dict.pop("interaction_id")
    ...
    results_generator = httpserver_manager.generate(adapter_dir, prompt, sampling_params, request_id, sys_len, app, input99app, sys99app, output99app, priorityfactor, llmcalls, app_limit, llmcalls_made, interaction_id)
...
def print_mem_stats(args):
    ...
    print(f"args.batch_max_tokens: {args.batch_max_tokens}")
    print(f"----------------------------------")
...
def main():
    ...
    # parser.add_argument("--max_req_input_len", type=int, default=4096,
    #                     help="the max value for req input tokens num")
    # parser.add_argument("--max_req_total_len", type=int, default=4096 + 1024,
    #                     help="the max value for req_input_len + req_output_len")
    ...
    if args.batch_max_tokens is None:
        ...
        args.batch_max_tokens = batch_max_tokens #3072
    else:
        assert (
    ...
    print(f"args.model_dir: {args.model_dir}, args.tokenizer_mode: {args.tokenizer_mode}. router_port: {router_port}, detokenization_port: {detokenization_port}, httpserver_port: {httpserver_port}, model_rpc_ports: {model_rpc_ports}, args.mode: {args.mode}, args.max_total_token_num: {args.max_total_token_num}, args.max_req_input_len: {args.max_req_input_len}, args.trust_remote_code: {args.trust_remote_code}, args.dummy: {args.dummy}")
